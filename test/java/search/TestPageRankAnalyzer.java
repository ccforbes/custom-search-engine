...
public class TestPageRankAnalyzer extends BaseTest {
    ...
    public void testSpecExample3() {
        ...
        URI pageFUri = URI.create("http://example.com/page-f.html");
        ...
        assertEquals(0.0, analyzer.computePageRank(pageFUri), DELTA);
    }
    
    @Test(timeout=SECOND)
    public void testSpecExample3LimitZero() {
        URI pageAUri = URI.create("http://example.com/page-a.html");
        URI pageBUri = URI.create("http://example.com/page-b.html");
        URI pageCUri = URI.create("http://example.com/page-c.html");
        URI pageDUri = URI.create("http://example.com/page-d.html");
        ...
        PageRankAnalyzer analyzer = new PageRankAnalyzer(pages, 0.85, 0.000001, 0);

        assertEquals(0.2, analyzer.computePageRank(pageAUri), DELTA);
        assertEquals(0.2, analyzer.computePageRank(pageBUri), DELTA);
        assertEquals(0.2, analyzer.computePageRank(pageCUri), DELTA);
        assertEquals(0.2, analyzer.computePageRank(pageDUri), DELTA);
        assertEquals(0.2, analyzer.computePageRank(pageEUri), DELTA);
        assertEquals(0.0, analyzer.computePageRank(pageFUri), DELTA);
    }
}
